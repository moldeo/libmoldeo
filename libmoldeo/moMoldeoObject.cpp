...
moMoldeoObject::ResolveValue( moParam& param, int value_index, bool p_refresh ) {
  ...
  switch( param_type ) {
      ...
      case MO_PARAM_VIDEO:
          ///ojo aqu� el video es tratado por el VideoManager si quiere ser tratado realamente
...
moMoldeoObject::CreateConnectors() {
  ...
  MODebug2->Message("moMoldeoObject::CreateConnectors > Calling once. object: "+GetName()+ " config: " + GetConfigName() + " label:" + GetLabelName() );
	...
	///crea los Inlets adicionales a los par�metros: definidos en el par�metro "inlet"
    ...
    if ( GetInletIndex(pinlets[i][MO_INLET_NAME].Text())==-1 ) {
      ...
      if (Inlet) {
        ...
        ///lo creamos si y solo si no existe como par�metro....
	...
	///Inicializa las funciones matem�ticas del config
	///as� como los inlets y outlets por cada par�metro
	///as� como las texturas
  ...
  MODebug2->Message("moMoldeoObject::CreateConnectors > loaded params & values for Object: " + GetName() + " config:" + GetConfigName() + " label:" + GetLabelName() );
    ...
    */
    ...
    /// Crea aquellos Outlets definidos dentro del par�metro "outlet"
    /// y conecta aquellos nombrados que ya existen como par�metros de este config
...
moMoldeoObject::Update( moEventList* p_EventList ) {
    ...
    if (poutlet) {
      ...
      if ( poutlet->Updated() || forcingParameterEmition ) {
        ...
        ///par�metros, por ejemplo el alpha.. o el translatex
...
int moMoldeoObject::luaGetTrackerFeaturesCount(moLuaVirtualMachine& vm) {
    ...
    ///este deber�a devolver -1 si no existe un dato de trackeo disponible...
    lua_State *state = (lua_State *) vm;
    int ft_count = -1;
