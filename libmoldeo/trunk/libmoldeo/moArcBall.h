 ...
 */
inline  static void
Point2fAdd (Point2fT * NewObj, const Tuple2fT * t1)
{
 ...
 */
inline  static void
Point2fSub (Point2fT * NewObj, const Tuple2fT * t1)
{
...
Matrix3fSetZero (Matrix3fT * NewObj)
{
	NewObj->s.M00 = NewObj->s.M01 = NewObj->s.M02 = NewObj->s.M10 =
		NewObj->s.M11 = NewObj->s.M12 = NewObj->s.M20 = NewObj->s.M21 =
		NewObj->s.M22 = 0.0f;
}
...
Matrix3fSetRotationFromQuat4f (Matrix3fT * NewObj, const Quat4fT * q1)
{
	...
	////assert(NewObj && q1);
	n =
		(q1->s.X * q1->s.X) + (q1->s.Y * q1->s.Y) + (q1->s.Z * q1->s.Z) +
		(q1->s.W * q1->s.W);
...
Matrix4fMulRotationScale (Matrix4fT * NewObj, GLfloat scale)
{
...
// 8<--Snip here if you have your own math types/funcs-->8
/// Objeto para la rotacin esfrica de la vista 3d
/**
...
{
	protected:inline
			void _mapToSphere (const Point2fT * NewPt,
			Vector3fT * NewVec) const;
	public:
		//Create/Destroy
		ArcBall_t ();
	...
	protected:Vector3fT StVec;	 //Saved click vector
