...
/**
 *   enumeracin de resoluciones de video
 */
enum moVideoMode
{								 //720 x 576
	PAL_D1 = 414720,
...
/**
 *   enumeracin de formatos de color
 */
enum moColorMode
{
	YUV = 0, RGB = 1
...
enum moStreamState
{
	...
	-1, MO_STREAMSTATE_PAUSED, MO_STREAMSTATE_PLAYING,
	MO_STREAMSTATE_WAITING, MO_STREAMSTATE_READY, MO_STREAMSTATE_STOPPED
};

/// Formato de video
/**
 *   estructura que define el ancho, alto, velocidad de reproduccin
 *   tamao de buffer, etc... de 1 cuadro de video
 */
class LIBMOLDEO_API moVideoFormat
{
	public:moVideoFormat ()
	{
	...
	MOuint m_TimePerFrame;		 /** duracin del frame en unidades de 100 nanosegundos (DirectShow) o 1ns (GStreamer), 1 ns = 1e-9 100ns = 1e-7 */
	MOuint m_BitCount;			 /** bits por cada pixel, 32, 24, 16, 15, 8, 1*/
	MOuint m_BufferSize;		 /** tamao en bytes del buffer de imagen*/
	MOuint m_FrameRate;			 /** cantidad de frames por cada 100 segundos, PAL: 2500 NTSC: 2997*/
	MOboolean m_WaitForFormat;	 /** bandera que marca la indefinicin del formato, default: true*/
};
...
class LIBMOLDEO_API moVideoSample
{
	public:
		...
		/**
		 *   Atencin: esta funcin no copia el buffer sino solamente
...
/// Definicin de un dispositivo de video, generalmente uno de captura de video, o camara
/**
 *   Un dispositivo de captura de video tiene 3 parametros, un nombre, una descricpin y una
 *   cadena  o cdigo de caracteres que lo representa.
 */
class LIBMOLDEO_API moCaptureDevice
{
	public:
		...
		moCaptureDevice ()
		{
			...
			m_DevicePort = 0;
			m_bPresent = false;
			m_SourceWidth = m_SourceHeight = m_SourceBpp = m_SourceFlipH =
				m_SourceFlipV = 0;
		}
		/// contructor
		moCaptureDevice (const moText & p_name, const moText & p_description,
			const moText & p_path, MOint p_deviceport =
			0, MOint p_sourcewidth = 0, MOint p_sourceheight =
			0, MOint p_bpp = 0, MOint p_fliph = 0, MOint p_flipv = 0)
		{
			m_Name = p_name;
		...
		/// Devuelve la descripcin del dispositivo
		...
		/// Seala y verifica si est presente el dispositivo
		bool IsPresent ()
		{
			return m_bPresent;
		}

		/// Fija el nombre de cdigo del dispositivo
		void SetCodeName (const moText & p_codename)
		{
			m_CodeName = p_codename;
		}
		/// Devuelve el nombre de cdigo del dispositivo
		moText GetCodeName ()
		{
			return m_CodeName;
		}
		...
		/// Devuelve el valor de inversin de imagen horizontal
		int GetSourceFlipH ()
		{
			return m_SourceFlipH;
		}

		/// Devuelve el valor de inversin de imagen vertical
		int GetSourceFlipV ()
		{
	...
	private:bool m_bPresent;	 ///Presencia del dispositivo
	moText m_Name;				 ///Nombre del dispositivo
	moText m_Description;		 ///Descripcin del dispositivo
	...
	MOint m_SourceFlipH;
	MOint m_SourceFlipV;
	moText m_CodeName;			 ///Cdigo del dispositivo
};

moDeclareExportedDynamicArray (moCaptureDevice, moCaptureDevices);

/// Plataforma de reproduccin de video, actualmente GStreamer para todas las plataformas
/**
...
class LIBMOLDEO_API moVideoFramework:public moAbstract
{
	...
	/**
	 *   devuelve una referencia a los dispositivos de video disponibles
	 *   esta funcin debe ser implementada en cada plataforma
	 */
	...
	/// Chequea si el dispositivos de video disponible est an disponible
	/**
	 *   En el indice i del arreglo de los dispositivos de video disponibles
	 *   confirma la disponibilidad de este.
	 *   Al implementar esta funcin se puede conectar y reconectar cmaras.
	 *   @return verdadero si est disponible, falso si no
	 */
	virtual bool CheckCaptureDevice (int i) = 0;

	/// Agrega un dispositivo de video
	/**
	 *   Si el dispositivo existe entonces esta funcin devuelve verdadero.
	...
	} protected:moCaptureDevices m_CaptureDevices;
	///Dispositivos de video disponibles
								 ///Dispositivos de video preferidos
	moCaptureDevices m_PreferredDevices;
};

/// Grafo de reproduccin de video
/**
...
class LIBMOLDEO_API moVideoGraph:public moAbstract
{
	...
	/// Finalizacin del grafo
	/**
	 *   Destruye los punteros y estructuras de la clase creadas dinmicamente en la funcin InitGraph
	 *   @return verdadero si fue exitoso, falso sino
	 */
	...
	/**
	 *   genera un grafo para capturar una cmara, especificada por moCaptureDevice
	 *   @return verdadero si fue exitoso, falso sino
	 */
	virtual bool BuildLiveGraph (moBucketsPool * pBucketsPool,
		moCaptureDevice p_capdev) = 0;

	/// Grafo de reproduccin de video en modo vivo, asyncronicamente reproducido en funcin del clock
	/**
	 *   genera un grafo para reproducir un video en modo normal
	 *   @return verdadero si fue exitoso, falso sino
	 */
	virtual bool BuildLiveVideoGraph (moText filename,
		moBucketsPool * pBucketsPool) = 0;
	virtual bool BuildLiveQTVideoGraph (moText filename,
		moBucketsPool * pBucketsPool) = 0;
	 ...
	 *   @return verdadero si fue exitoso, falso sino
	 */
	virtual bool BuildRecordGraph (moText filename,
		moBucketsPool * pBucketsPool) = 0;
	...
	/// Reproducir el video
	/**
	 *   comando de play, reproduce el video o continua la reproduccin
	 */
	virtual void Play () = 0;

	/// Detener la reproduccin del video
	/**
	 *   comando de stop, detiene la reproduccin del video
	 */
	virtual void Stop () = 0;

	/// Pausa la reproduccin del video
	/**
	 *   comando de pause, interrumpe la reproduccin del video
	 */
	virtual void Pause () = 0;

	/// Busca y posiciona
	/**
	 *   comando de Seek, salta a la posicin deseada y luego queda en pausa
	...
	/// Est corriendo
	/**
	 *   indicador que seala si el grafo se est reproduciendo
	...
	/**
	 *   indicador que seala el modo de video del video que se reproduce
	...
	/**
	 *   indicador que seala el formato de video utilizado por esta entrada de video
	 *   @return moVideoFormat el formato de video
	 */
	moVideoFormat GetVideoFormat ();
	virtual moStreamState GetState ();
	moText StateToText (moStreamState state);
	protected:moVideoFormat m_VideoFormat;
	///Formato de video
};
#endif							 /*  */
