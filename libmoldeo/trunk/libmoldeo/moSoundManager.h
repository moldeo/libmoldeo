...
enum moSoundType
{
	MO_SOUNDTYPE_UNDEFINED, MO_SOUNDTYPE_MP3, MO_SOUNDTYPE_WAV,
	MO_SOUNDTYPE_OGG
};
...
class LIBMOLDEO_API moSound:public moAbstract
{
	public:moSound ();
	virtual ~ moSound ();
	virtual MOboolean Init (moText p_name, MOuint p_moid, moSoundType p_type,
		moResourceManager * p_res, moSoundParam p_param =
		MODefSoundParams);
	...
	float GetVolume ();
	protected:MOint m_ActualSample;
	MOint m_OldSample;
	moFile * m_pFile;
...
class LIBMOLDEO_API moSoundManager:public moResource
{
	...
	protected:moSound * CreateSound (moText p_name, moSoundType p_type,
			moSoundParam p_param);
