...
namespace OGLFT
{
	...
	#ifdef WIN32
	typedef void (__stdcall * GLUTessCallback) (void);
	#else
	...
	class Face
	{
		...
		private:
			...
			struct FaceData
			{
				FT_Face face_;
				bool free_on_exit_;
				FaceData (FT_Face face, bool free_on_exit = true):face_ (face),
					free_on_exit_
					(free_on_exit)
				{
				}
		...
		public:
			 ...
			 * Defaults to 100 DPI.
			 */
			Face (const char *filename, float point_size = 12, FT_UInt resolution =
				100);
			...
			void setForegroundColor (GLfloat red = 0.0,
				GLfloat green = 0.0,
				GLfloat blue = 0.0, GLfloat alpha = 1.0);
			...
			void setBackgroundColor (GLfloat red = 1.0,
				GLfloat green = 1.0,
				GLfloat blue = 1.0, GLfloat alpha = 0.0);
			...
			{
				return vertical_justification_;
			}
			...
			int ascender (void)
			{
				return faces_.front ().face_->ascender;
			}
			...
			int descender (void)
			{
				return faces_.front ().face_->descender;
			}
	...
	class Polygonal:public Face
	{
		protected:
			 ...
			 */
			struct LIBMOLDEO_API VertexInfo
			{
				...
				VertexInfo (ColorTess * color_tess = 0, TextureTess * texture_tess = 0):color_tess_ (color_tess),
					texture_tess_
					(texture_tess)
				{
					v_[X] = v_[Y] = v_[Z] = 0.;
				 ...
				 * \param texture_tess optional texture tesselation object.
				 */
				VertexInfo (FT_Vector * ft_v, ColorTess * color_tess = 0, TextureTess * texture_tess = 0):color_tess_ (color_tess),
					texture_tess_
					(texture_tess)
				{
				 ...
				 * \param texture_tess optional texture tesselation object.
				 */
				VertexInfo (double p[2], ColorTess * color_tess = 0, TextureTess * texture_tess = 0):color_tess_ (color_tess),
					texture_tess_
					(texture_tess)
				{
					v_[X] = p[X];
				 ...
				 * \param texture_tess optional texture tesselation object.
				 */
				VertexInfo (double x, double y, ColorTess * color_tess = 0, TextureTess * texture_tess = 0):color_tess_ (color_tess),
					texture_tess_
					(texture_tess)
				{
					v_[X] = x;
	...
	class Outline:public Polygonal
	{
		...
		private:
			...
			static int conicToCallback (FT_Vector * control, FT_Vector * to,
				Outline * outline);
	...
	class Solid:public Filled
	{
		...
		public:
			 ...
			 * Defaults to 100 DPI.
			 */
			Solid (const char *filename, float point_size = 12, FT_UInt resolution =
				100);
	...
	class Raster:public Face
	{
		...
		public:
			 ...
			 * Defaults to 100 DPI.
			 */
			Raster (const char *filename, float point_size = 12, FT_UInt resolution =
				100);
			/*!
			 * \param face open FreeType FT_Face.
			 * \param point_size the initial point size of the font to generate. A point
	...
	class Monochrome:public Raster
	{
		public:
			 ...
			 * Defaults to 100 DPI.
			 */
			Monochrome (FT_Face face, float point_size = 12, FT_UInt resolution =
				100);
			/*!
			 * The destructor doesn't do anything in particular.
			 */
	...
	class Translucent:public Raster
	{
		public:
			 ...
			 * Defaults to 100 DPI.
			 */
			Translucent (FT_Face face, float point_size = 12, FT_UInt resolution =
				100);
			...
			struct TextureInfo
			{
				...
				int width_,		 //!< The 2**l width of the texture.
					height_;	 //!< The 2**m height of the texture.
								 //!< The fraction of the texture width occupied
				GLfloat texture_s_,
				//!< by the glyph.
	...
	class MonochromeTexture:public Texture
	{
		...
		private:
			GLubyte * invertBitmap (const FT_Bitmap & bitmap, int *width,
				int *height);
	...
	class GrayscaleTexture:public Texture
	{
		...
		private:
			GLubyte * invertPixmap (const FT_Bitmap & bitmap, int *width,
				int *height);
			void bindTexture (FT_Face face, FT_UInt glyph_index);
	};
