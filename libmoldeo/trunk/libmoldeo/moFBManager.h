 ...
 * tal que pueda ser restaurado cuando al llamar a la funcin de desactivacin (unbind).
...
class LIBMOLDEO_API moFBManager:public moResource
{
	public:
		...
		/**
		 * Agrega un nuevo FBO.
		 * @return ndice del nuevo FBO.
		 */
		MOuint CreateFBO ();

		/**
		 * Elimina el FBO con ndice p_fbo.
		 * @param p_fbo ndice del FBO a eliminar.
		 * @return true en caso de que el FBO se haya podido eliminar, false en caso contrario.
		 */
		MOboolean DeleteFBO (MOuint p_fbo);

		/**
		 * Devuelve el nmero de FBOs.
		 * @return nmero de FBOs.
		 */
		MOuint GetFBOCount ()
		{
			return m_fbo_array.Count ();
		}

		/**
		 * Devuelve un puntero al FBO con ndice p_fbo.
		 * @param p_fbo ndice del FBO.
		 * @return puntero al FBO.
		 */
		moFBO *GetFBO (MOuint p_fbo)
		{
			return m_fbo_array.Get (p_fbo);
		}

		/**
		 * Devuelve un puntero al primer FBO cuyos parmetros coincidan con p_target, p_internal_format, p_width
		 * y p_height.
		 * @param p_target trget OpenGL de las texturas asociadas al FBO (GL_TEXTURE_2D, GL_TEXTURE_RECT, etc.).
		 ...
		 * @return puntero al FBO.
		 */
		MOint GetFBO (MOuint p_target, MOuint p_internal_format, MOuint p_width,
			MOuint p_height);
		 ...
		 * @param p_fbo ndice del FBO.
		 * @return true si la operacin tiene xito, false en caso contrario.
		 */
		 ...
		 * @param p_fbo ndice del FBO.
		 * @param p_num_attach_points nmero de texturas de escritura a utilizar.
		 * @param p_attach_point_list ndice de los bfers de color a utilizar (a los cuales estn asociados las texturas).
		 * @return true si la operacin tiene xito, false en caso contrario.
		 */
		MOboolean BindFBO (MOuint p_fbo, MOuint p_num_attach_points,
			MOuint * p_attach_point_list);

		/**
		 * Agrega la textura apuntada por p_pTex como nuevo bfer de color del FBO con ndice p_fbo.
		 * @param p_fbo ndice del FBO.
		 ...
		 */
		MOboolean AddTexture (MOuint p_fbo, moTexture * p_pTex,
			MOuint & p_attach_point);

		/**
		 * Asocia la textura apuntada por p_pTex al bfer de color p_attach_point del FBO con ndice p_fbo.
		 * @param p_fbo ndice del FBO.
		 * @param p_pTex puntero a la textura a agregar.
		 * @param p_attach_point ndice del punto de asociacin al cual ser asociada la textura.
		 * @return resultado de la operacin de asociacin.
		 */
		MOboolean AttachTexture (MOuint p_fbo, moTexture * p_pTex,
			MOuint p_attach_point);

		/**
		 * Establece como textura de escritura aquella asociada en el bfer de color con ndice p_attach_point
		...
		/**
		 * Establece como texturas de escritura aquellas asociada con los bfers de color con ndice p_attach_point_list
		 * en el FBO actualmente activo.
		 * @param p_num_attach_points nmero de texturas de escritura a utilizar.
		 * @param p_attach_point_list ndice de los bfers de color a utilizar (a los cuales estn asociados las texturas).
		 */
		void SetDrawTextures (MOuint p_num_attach_points,
			MOuint * p_attach_point_list);

		/**
		 * Establece como textura de lectura aquella asociada en el bfer de color con ndice p_attach_point
		 * en el FBO actualmente activo.
		 * @param p_attach_point ndice del bfer de color a utilizar.
		 */
		 ...
		 * @param p_param parmetros de la textura.
		 * @param p_def_attach_point ndice del punto de asociacin por defecto.
		 */
		void SetDrawTextureByGLId (MOuint p_glid, MOuint p_width, MOuint p_height,
		 ...
		 * @param p_num_glids nmero de texturas a utilizar.
		 ...
		 * @param p_param parmetros de las texturas.
		 */
		void SetDrawTexturesByGLId (MOuint p_num_glids, MOuint * p_glid_list,
			MOuint p_width, MOuint p_height,
			const moTexParam & p_param);
		 ...
		 * @param p_def_attach_point ndice del punto de asociacin por defecto.
		 */
		void SetReadTextureByGLId (MOuint p_glid, MOuint p_width, MOuint p_height,
			const moTexParam & p_param,
			MOuint p_def_attach_point = 0);

		/**
		 * Establece como textura de escritura aquella apuntada por p_pTex. Si dicha textura no est asociada
		 * al FBO actualmente activo, la funcin devuelve false.
		 * @param p_pTex puntero a la textura a utilizar.
		 * @param p_def_attach_point ndice del punto de asociacin por defecto.
		 * @return true si la operacin es exitosa, false en caso contrario.
		 */
		...
		/**
		 * Establece como textura de lectura aquella apuntada por p_pTex. Si dicha textura no est asociada
		 * al FBO actualmente activo, la funcin devuelve false.
		 * @param p_pTex puntero a la textura a utilizar.
		 * @param p_def_attach_point ndice del punto de asociacin por defecto.
		 * @return true si la operacin es exitosa, false en caso contrario.
