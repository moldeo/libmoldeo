...
void moTextureFilter::SetupShader(MOint w, MOint h, moTempo *p_tempo, MOfloat p_fade, moTextFilterParam *p_params)
{
	...
	if (-1 < m_tempo_angle)
		if (p_tempo != NULL)
		{
			...
			(m_shader->GetType() == MO_SHADER_GLSL) ? glUniform2fARB(m_tempo_angle, a, f) : break/*cgGLSetParameter2f( (CGparameter)m_tempo_angle, a, f)*/;
		}
		else {
		    //glUniform2fARB(m_tempo_angle, 0.0, 0.0);
		    (m_shader->GetType() == MO_SHADER_GLSL) ? glUniform2fARB(m_tempo_angle, 0.0, 0.0) : break/*cgGLSetParameter2f( (CGparameter)m_tempo_angle, 0.0, 0.0 )*/;
		}

	if (-1 < m_dest_tex_size) {
	   // glUniform2fARB(m_dest_tex_size, w, h);
	   (m_shader->GetType() == MO_SHADER_GLSL) ? glUniform2fARB(m_dest_tex_size, w, h) : break/*cgGLSetParameter2f( (CGparameter)m_dest_tex_size, w, h )*/;
	}

	if (-1 < m_fade_const) {
        //glUniform1fARB(m_fade_const, p_fade);
	    (m_shader->GetType() == MO_SHADER_GLSL) ? glUniform1fARB(m_fade_const, p_fade) : break/*cgGLSetParameter1f( (CGparameter)m_fade_const, p_fade )*/;
	}
