...
inline Complex * FFTWComplex (size_t size)
{
	static const
		size_t
		offset = sizeof (size_t) / sizeof (Complex) +
		(sizeof (size_t) % sizeof (Complex) > 0);
	void *
		alloc = fftw_malloc ((size + offset) * sizeof (Complex));
	if (size && !alloc)
...
FFTWdouble (size_t size)
{
	static const
		...
		(sizeof (size_t) % sizeof (Complex) > 0);
	void *
		alloc = fftw_malloc (size * sizeof (double) + offset * sizeof (Complex));
	if (size && !alloc)
		cerr << endl << "Memory limits exceeded" << endl;
	*(size_t *) alloc = size;
	double *
		p = (double *) ((Complex *) alloc + offset);
	for (size_t i = 0; i < size; i++)
		new (p + i) double;
	return p;
}
...
inline void
fftw_export_wisdom (void (*emitter) (char c, ofstream & s), ofstream & s)
{
	fftw_export_wisdom ((void (*)(char, void *)) emitter, (void *) &s);
} inline int

fftw_import_wisdom (int (*g) (ifstream & s), ifstream & s)
{
	return fftw_import_wisdom ((int (*)(void *)) g, (void *) &s);
} inline void

PutWisdom (char c, ofstream & s)
{
	s.put (c);
} inline int

GetWisdom (ifstream & s)
{
	return s.get ();
}
...
class fftw
{
	protected:unsigned int
		...
		static
			...
			ofWisdom;
		unsigned int
			Dist (unsigned int n, unsigned int stride, unsigned int dist)
		{
			return dist ? dist : ((stride == 1) ? n : 1);
		}
		unsigned int
			realsize (unsigned int n, Complex * in, Complex * out)
		{
			return n / 2 + (!out || in == out);
		}
		unsigned int
			realsize (unsigned int n, Complex * in, double *out)
		{
		...
		void
			Shift (Complex * data, unsigned int nx, unsigned int ny)
		{
			const unsigned int
				nyp = ny / 2 + 1;
		...
		void
			Shift (Complex * data, unsigned int nx, unsigned int ny, unsigned int nz)
		{
			const unsigned int
				nzp = nz / 2 + 1;
			const unsigned int
		...
		} public: fftw (unsigned int size, int sign, unsigned int n = 0):size (size), sign (sign), norm (1.0 / (n ? n : size)),
			shift
			(false)
		{
		} virtual ~ fftw ()
		...
		inline void
			CheckAlign (Complex * p, const char *s)
		{
		...
		void
			Setup (Complex * in, Complex * out = NULL)
		{
			if (!Wise)
				LoadWisdom ();
			bool alloc = !in;
			...
			if (out)
				CheckAlign (out, "constructor output");
		...
		void
			Setup (Complex * in, double *out)
		{
			Setup (in, (Complex *) out);
		} void
			Setup (double *in, Complex * out)
		{
			...
			Execute (Complex * in, Complex * out)
		{
			...
			Setout (Complex * in, Complex * &out)
		{

		#ifndef NO_CHECK_ALIGN
			CheckAlign (in, "input");
			if (out)
				CheckAlign (out, "output");
			...
			{
				cerr <<
					"ERROR: fft constructor and call must be either both in place or out of place"
					<< endl;
				exit (1);
			}
		}
		void
			fft (Complex * in, Complex * out = NULL)
		{
			Setout (in, out);
			Execute (in, out);
		} void
			fft (double *in, Complex * out)
		{
			fft ((Complex *) in, out);
		} void
			fft (Complex * in, double *out)
		{
			fft (in, (Complex *) out);
		} void
			fft0 (Complex * in, Complex * out = NULL)
		{
			Setout (in, out);
			shift = true;
			Execute (in, out);
			...
			fft0 (double *in, Complex * out)
		{
			fft0 ((Complex *) in, out);
		} void
			fft0 (Complex * in, double *out)
		{
			fft0 (in, (Complex *) out);
		} void
			Normalize (Complex * out)
		{
			...
			fftNormalized (Complex * in, Complex * out = NULL)
		{
			Setout (in, out);
			Execute (in, out);
			Normalize (out);
		} void
			fftNormalized (Complex * in, double *out)
		{
			fftNormalized (in, (Complex *) out);
		} void
			fftNormalized (double *in, Complex * out)
		{
			fftNormalized ((Complex *) in, out);
		} void
			fft0Normalized (Complex * in, Complex * out = NULL)
		{
			Setout (in, out);
			shift = true;
			Execute (in, out);
			...
			fft0Normalized (Complex * in, double *out)
		{
			fft0Normalized (in, (Complex *) out);
		} void
			fft0Normalized (double *in, Complex * out)
		{
			fft0Normalized ((Complex *) in, out);
		} void
			fftNormalized (Complex * in, Complex * out, unsigned int nx,
			unsigned int m, unsigned int stride, unsigned int dist)
		{
			if (stride == 1 && dist == nx)
				fftw::fftNormalized (in, out);

			else if (stride == nx && dist == 1)
				fftw::fftNormalized (in, out);
...
class fft1d:public fftw
{
	unsigned int
		nx;
	public: fft1d (unsigned int nx, int sign, Complex * in = NULL, Complex * out = NULL) :fftw (nx, sign),
			nx
			(nx)
		{
			Setup (in, out);
...
class mfft1d:public fftw
{
	...
	unsigned int
		dist;
	public: mfft1d (unsigned int nx, int sign, unsigned int m = 1, unsigned int stride = 1, unsigned int dist = 0, Complex * in = NULL, Complex * out = NULL) :fftw ((nx - 1) * stride + (m - 1) * Dist (nx, stride, dist) + 1, sign, nx),
			nx (nx), m (m), stride (stride),
			dist (Dist (nx, stride, dist))
		{
			Setup (in, out);
		} fftw_plan Plan (Complex * in, Complex * out)
		{
...
class rcfft1d:public fftw
{
	unsigned int
		nx;
	public: rcfft1d (unsigned int nx, Complex * out = NULL) :fftw (nx / 2 + 1, -1, nx),
			nx (nx)
		{
			Setup (out);
		} rcfft1d (unsigned int nx, double *in, Complex * out = NULL) :fftw (nx / 2 + 1, -1, nx),
			nx (nx)
		{
			Setup (in, out);
		}
...
class crfft1d:public fftw
{
	unsigned int
		nx;
	public: crfft1d (unsigned int nx, Complex * in = NULL) :fftw (nx / 2 + 1, 1, nx),
			nx (nx)
		{
			Setup (in);
		} crfft1d (unsigned int nx, Complex * in, double *out) :
		fftw (realsize (nx, in, out), 1, nx),
			nx (nx)
		{
			Setup (in, out);
		}
...
class mrcfft1d:public fftw
{
	...
	unsigned int
		dist;
	public: mrcfft1d (unsigned int nx, unsigned int m = 1, unsigned int stride = 1, unsigned int dist = 0, Complex * out = NULL) :fftw (nx / 2 * stride + (m - 1) * Dist (nx, stride, dist) + 1, -1, nx),
			nx (nx), m (m), stride (stride),
			dist (Dist (nx, stride, dist))
		{
			Setup (out);
		} mrcfft1d (unsigned int nx, unsigned int m = 1, unsigned int stride = 1, unsigned int dist = 0, double *in = NULL, Complex * out = NULL) :fftw (nx / 2 * stride + (m - 1) * Dist (nx, stride, dist) + 1, -1, nx),
			nx (nx), m (m), stride (stride),
			dist (Dist (nx, stride, dist))
		{
			Setup (in, out);
...
class mcrfft1d:public fftw
{
	...
	unsigned int
		dist;
	public: mcrfft1d (unsigned int nx, unsigned int m = 1, unsigned int stride = 1, unsigned int dist = 0, Complex * in = NULL) :fftw (nx / 2 * stride + (m - 1) * Dist (nx, stride, dist) + 1, 1, nx),
			nx (nx), m (m), stride (stride),
			dist (Dist (nx, stride, dist))
		{
			Setup (in);
		} mcrfft1d (unsigned int nx, unsigned int m = 1, unsigned int stride = 1, unsigned int dist = 0, Complex * in = NULL, double *out = NULL) :fftw ((realsize (nx, in, out) - 1) * stride +
			(m - 1) * Dist (nx, stride, dist) + 1, 1, nx), nx (nx), m (m),
			stride (stride), dist (Dist (nx, stride, dist))
		{
			Setup (in, out);
		} fftw_plan Plan (Complex * in, Complex * out)
		{
...
class fft2d:public fftw
{
	...
	unsigned int
		ny;
	public: fft2d (unsigned int nx, unsigned int ny, int sign, Complex * in = NULL, Complex * out = NULL) :fftw (nx * ny, sign), nx (nx),
			ny
			(ny)
		{
			Setup (in, out);
		...
		fftw_plan Plan (Complex * in, Complex * out)
		{
			return fftw_plan_dft_2d (nx, ny, (fftw_complex *) in,
...
class rcfft2d:public fftw
{
	...
	unsigned int
		ny;
	public: rcfft2d (unsigned int nx, unsigned int ny, Complex * out = NULL):fftw (nx * (ny / 2 + 1), -1, nx * ny), nx (nx),
			ny
			(ny)
		{
			Setup (out);
		} rcfft2d (unsigned int nx, unsigned int ny, double *in, Complex * out = NULL):fftw (nx * (ny / 2 + 1), -1, nx * ny), nx (nx),
			ny
			(ny)
		{
			Setup (in, out);
		...
		fftw (in.Size (), -1, in.Nx () * 2 * (in.Ny () - 1)),
			nx (in.Nx ()),
			ny (2 * (in.Ny () - 1))
		{
			Setup (in);
...
class crfft2d:public fftw
{
	...
	unsigned int
		ny;
	public: crfft2d (unsigned int nx, unsigned int ny, Complex * in = NULL) :fftw (nx * (ny / 2 + 1), 1, nx * ny), nx (nx),
			ny
			(ny)
		{
			Setup (in);
		} crfft2d (unsigned int nx, unsigned int ny, Complex * in, double *out) :
		fftw (nx * (realsize (ny, in, out)), 1, nx * ny),
			nx (nx),
			ny (ny)
		{
			Setup (in, out);
...
class fft3d:public fftw
{
	...
	unsigned int
		nz;
	public: fft3d (unsigned int nx, unsigned int ny, unsigned int nz, int sign, Complex * in = NULL, Complex * out = NULL) :fftw (nx * ny * nz, sign), nx (nx), ny (ny),
			nz
			(nz)
		{
			Setup (in, out);
...
class rcfft3d:public fftw
{
	...
	unsigned int
		nz;
	public: rcfft3d (unsigned int nx, unsigned int ny, unsigned int nz, Complex * out = NULL) :fftw (nx * ny * (nz / 2 + 1), -1, nx * ny * nz), nx (nx), ny (ny),
			nz
			(nz)
		{
			Setup (out);
		} rcfft3d (unsigned int nx, unsigned int ny, unsigned int nz, double *in, Complex * out = NULL):fftw (nx * ny * (nz / 2 + 1), -1, nx * ny * nz), nx (nx), ny (ny),
			nz
			(nz)
		{
			Setup (in, out);
...
class crfft3d:public fftw
{
	...
	unsigned int
		nz;
	public: crfft3d (unsigned int nx, unsigned int ny, unsigned int nz, Complex * in = NULL) :fftw (nx * ny * (nz / 2 + 1), 1, nx * ny * nz), nx (nx), ny (ny),
			nz
			(nz)
		{
			Setup (in);
			...
			double *out):
		fftw (nx * ny * (realsize (nz, in, out)), 1, nx * ny * nz),
			nx (nx),
			ny (ny),
			nz (nz)
		{
