...
// moMatrix2 class ------------------------------------------------------------
template < class Real > class LIBMOLDEO_API moMatrix2:public moAbstract
{
	public:moMatrix2 (bool bZero = true)
	{
		if (bZero)
		{
			MakeZero ();
	...
	{
		return moMatrix2 < Real > (m_afEntry[0],
			m_afEntry[2], m_afEntry[1], m_afEntry[3]);
	}
	...
	{
		return moMatrix2 < Real > (m_afEntry[3],
			-m_afEntry[1], -m_afEntry[2], m_afEntry[0]);
	}
	...
	{
		return moMatrix2 < Real > (m_afEntry[0] + rkM.m_afEntry[0],
			m_afEntry[1] + rkM.m_afEntry[1],
			m_afEntry[2] + rkM.m_afEntry[2],
			m_afEntry[3] + rkM.m_afEntry[3]);
	}
	...
	{
		return moMatrix2 < Real > (fScalar * m_afEntry[0],
			fScalar * m_afEntry[1],
			fScalar * m_afEntry[2],
			fScalar * m_afEntry[3]);
	}
	inline moMatrix2 operator/ (Real fScalar) const
	{
...

		{
			...
			return moMatrix2 < Real > (fInvScalar * m_afEntry[0],
				fInvScalar * m_afEntry[1],
				fInvScalar * m_afEntry[2],
				fInvScalar * m_afEntry[3]);
		}
		return moMatrix2 < Real > (moMath < Real >::MAX_REAL,
	...
	{
		return moMatrix2 < Real > (-m_afEntry[0], -m_afEntry[1],
			-m_afEntry[2], -m_afEntry[3]);
	}
	...
	// matrix times vector
								 // M * v
	inline moVector2 < Real > operator* (const moVector2 < Real > &rkV) const
	{
...
// c * M
template < class Real > inline LIBMOLDEO_API moMatrix2 < Real >
operator* (Real fScalar, const moMatrix2 < Real > &rkM)
{
	return rkM * fScalar;
}
...
// v^T * M
template < class Real > inline LIBMOLDEO_API moVector2 < Real >
operator* (const moVector2 < Real > &rkV, const moMatrix2 < Real > &rkM)
{
	return moVector2 < Real > (rkV[0] * rkM[0][0] + rkV[1] * rkM[1][0],
...
// moMatrix3 class ------------------------------------------------------------
template < class Real > class moMatrix3:public moAbstract
{
	public:moMatrix3 (bool bZero = true)
	{
		if (bZero)
		{
			MakeZero ();
	...
	{
		return moVector3 < Real > (m_afEntry[iCol], m_afEntry[iCol + 3],
			m_afEntry[iCol + 6]);
	}
	...
	{
		return moMatrix3 < Real > (m_afEntry[0],
			m_afEntry[3],
			m_afEntry[6],
			m_afEntry[1],
			m_afEntry[4],
	...
	{
		// P = A^T*B
		return moMatrix3 < Real > (m_afEntry[0] * rkM.m_afEntry[0] +
			m_afEntry[3] * rkM.m_afEntry[3] +
			m_afEntry[6] * rkM.m_afEntry[6],
	...
	Real Determinant () const
	{
		...
		Real fDet =
			m_afEntry[0] * fCo00 + m_afEntry[1] * fCo10 + m_afEntry[2] * fCo20;
		return fDet;
	}
	...
	{
		return moMatrix3 < Real > (m_afEntry[0] * rkDiag[0],
			m_afEntry[1] * rkDiag[1],
			m_afEntry[2] * rkDiag[2],
			m_afEntry[3] * rkDiag[0],
			m_afEntry[4] * rkDiag[1],
			m_afEntry[5] * rkDiag[2],
			m_afEntry[6] * rkDiag[0],
			m_afEntry[7] * rkDiag[1],
			m_afEntry[8] * rkDiag[2]);
	}

	moMatrix3 < Real > DiagonalTimes (const moVector3 < Real > &rkDiag) const
	{
		return moMatrix3 < Real > (rkDiag[0] * m_afEntry[0],
			rkDiag[0] * m_afEntry[1],
			rkDiag[0] * m_afEntry[2],
			rkDiag[1] * m_afEntry[3],
			rkDiag[1] * m_afEntry[4],
			rkDiag[1] * m_afEntry[5],
			rkDiag[2] * m_afEntry[6],
			rkDiag[2] * m_afEntry[7],
			rkDiag[2] * m_afEntry[8]);
	}
			...
			else
			{
				...
				if (m_afEntry[0] >= m_afEntry[4])
				{
					...
					if (m_afEntry[0] >= m_afEntry[8])
					{
						...
						rkAxis[0] =
							((Real) 0.5) * moMath <
							Real >::Sqrt (m_afEntry[0] - m_afEntry[4] -
							m_afEntry[8] + (Real) 1.0);
					...
					else
					{
						...
						rkAxis[2] =
							((Real) 0.5) * moMath <
							Real >::Sqrt (m_afEntry[8] - m_afEntry[0] -
							m_afEntry[4] + (Real) 1.0);
				...
				else
				{
					...
					if (m_afEntry[4] >= m_afEntry[8])
					{
						...
						rkAxis[1] =
							((Real) 0.5) * moMath <
							Real >::Sqrt (m_afEntry[4] - m_afEntry[0] -
							m_afEntry[8] + (Real) 1.0);
		...
		Real fZAngle)
	{
		...
		moMatrix3 kXMat ((Real) 1.0, (Real) 0.0, (Real) 0.0,
			(Real) 0.0, fCos, -fSin, (Real) 0.0, fSin, fCos);

		fCos = moMath < Real >::Cos (fYAngle);
		fSin = moMath < Real >::Sin (fYAngle);
		moMatrix3 kYMat (fCos, (Real) 0.0, fSin,
			(Real) 0.0, (Real) 1.0, (Real) 0.0,
			-fSin, (Real) 0.0, fCos);

		fCos = moMath < Real >::Cos (fZAngle);
		fSin = moMath < Real >::Sin (fZAngle);
		moMatrix3 kZMat (fCos, -fSin, (Real) 0.0,
			fSin, fCos, (Real) 0.0,
		...
		for (int i = 0; i < iMax; i++)
		{
			...
			if (bTest1)
			{
				...
				else
				{
					...
					fTan0 =
						((Real) 0.5) * (fTmp + moMath <
						Real >::Sqrt (fTmp * fTmp + ((Real) 4.0)));
	...
	void QDUDecomposition (moMatrix3 & rkQ, moMatrix3 & rkD, moMatrix3 & rkU) const
	{
		...
		Real fDet =
			rkQ[0][0] * rkQ[1][1] * rkQ[2][2] + rkQ[0][1] * rkQ[1][2] * rkQ[2][0] +
	...
	{
		return moMatrix3 < Real >
			(m_afEntry[0] * rkM.m_afEntry[0] + m_afEntry[1] * rkM.m_afEntry[3] +
			m_afEntry[2] * rkM.m_afEntry[6],
			m_afEntry[0] * rkM.m_afEntry[1] + m_afEntry[1] * rkM.m_afEntry[4] +
			m_afEntry[2] * rkM.m_afEntry[7],
			m_afEntry[0] * rkM.m_afEntry[2] + m_afEntry[1] * rkM.m_afEntry[5] +
			m_afEntry[2] * rkM.m_afEntry[8],
			m_afEntry[3] * rkM.m_afEntry[0] + m_afEntry[4] * rkM.m_afEntry[3] +
			m_afEntry[5] * rkM.m_afEntry[6],
			m_afEntry[3] * rkM.m_afEntry[1] + m_afEntry[4] * rkM.m_afEntry[4] +
			m_afEntry[5] * rkM.m_afEntry[7],
			m_afEntry[3] * rkM.m_afEntry[2] + m_afEntry[4] * rkM.m_afEntry[5] +
			m_afEntry[5] * rkM.m_afEntry[8],
			m_afEntry[6] * rkM.m_afEntry[0] + m_afEntry[7] * rkM.m_afEntry[3] +
			m_afEntry[8] * rkM.m_afEntry[6],
			m_afEntry[6] * rkM.m_afEntry[1] + m_afEntry[7] * rkM.m_afEntry[4] +
			m_afEntry[8] * rkM.m_afEntry[7],
			m_afEntry[6] * rkM.m_afEntry[2] + m_afEntry[7] * rkM.m_afEntry[5] +
			m_afEntry[8] * rkM.m_afEntry[8]);
	}
	inline moMatrix3 operator* (Real fScalar) const
	{
		return moMatrix3 < Real > (fScalar * m_afEntry[0],
			fScalar * m_afEntry[1],
			fScalar * m_afEntry[2],
			fScalar * m_afEntry[3],
			fScalar * m_afEntry[4],
	...
	inline moMatrix3 operator/ (Real fScalar) const
	{
		...
		{
			Real fInvScalar = ((Real) 1.0) / fScalar;
			return moMatrix3 < Real > (fInvScalar * m_afEntry[0],
				fInvScalar * m_afEntry[1],
				fInvScalar * m_afEntry[2],
				fInvScalar * m_afEntry[3],
		...
		return moMatrix3 < Real > (moMath < Real >::MAX_REAL,
			moMath < Real >::MAX_REAL,
			moMath < Real >::MAX_REAL,
			moMath < Real >::MAX_REAL,
			moMath < Real >::MAX_REAL,
	...
	{
		return moMatrix3 < Real > (-m_afEntry[0], -m_afEntry[1],
			-m_afEntry[2], -m_afEntry[3],
			-m_afEntry[4], -m_afEntry[5],
	...
	// matrix times vector
								 // M * v
	inline moVector3 < Real > operator* (const moVector3 < Real > &rkV) const
	{
	...
	private:
		...
		bool QLAlgorithm (Real afDiag[3], Real afSubd[2])
		{
			...
			for (int i = 0; i < iMax; i++)
			{
				...
				fSum =
					moMath < Real >::FAbs (afDiag[0]) + moMath < Real >::FAbs (afDiag[1]);
				if (moMath < Real >::FAbs (afSubd[0]) + fSum == fSum)
				{
					...
					for (iRow = 0; iRow < 3; iRow++)
					{
						...
						m_afEntry[2 + 3 * iRow] =
							fSin * m_afEntry[1 + 3 * iRow] + fCos * fTmp;
						m_afEntry[1 + 3 * iRow] =
							fCos * m_afEntry[1 + 3 * iRow] - fSin * fTmp;
					}
				...
				fSum =
					moMath < Real >::FAbs (afDiag[1]) + moMath < Real >::FAbs (afDiag[2]);
				if (moMath < Real >::FAbs (afSubd[1]) + fSum == fSum)
				{
					...
					for (iRow = 0; iRow < 3; iRow++)
					{
						...
						m_afEntry[1 + 3 * iRow] =
							fSin * m_afEntry[0 + 3 * iRow] + fCos * fTmp;
						m_afEntry[0 + 3 * iRow] =
							fCos * m_afEntry[0 + 3 * iRow] - fSin * fTmp;
					}
		...
		static void Bidiagonalize (moMatrix3 & rkA, moMatrix3 & rkL, moMatrix3 & rkR)
		{
			...
			fLength =
				moMath <
				Real >::Sqrt (rkA[0][0] * rkA[0][0] + rkA[1][0] * rkA[1][0] +
				rkA[2][0] * rkA[2][0]);
			if (fLength > (Real) 0.0)
			{
				...
				fT2 =
					-((Real) 2.0) / (((Real) 1.0) + afV[1] * afV[1] + afV[2] * afV[2]);
			...
			// map first row to (*,*,0)
			fLength =
				moMath < Real >::Sqrt (rkA[0][1] * rkA[0][1] + rkA[0][2] * rkA[0][2]);
			if (fLength > (Real) 0.0)
			{
			...
			fLength =
				moMath < Real >::Sqrt (rkA[1][1] * rkA[1][1] + rkA[2][1] * rkA[2][1]);
			if (fLength > (Real) 0.0)
			{
		...
		static void GolubKahanStep (moMatrix3 & rkA, moMatrix3 & rkL, moMatrix3 & rkR)
		{
			...
			Real fDiscr =
				moMath < Real >::Sqrt (fDiff * fDiff + ((Real) 4.0) * fT12 * fT12);
			...
			Real fY = rkA[0][0] - (moMath < Real >::FAbs (fRoot1 - fT22) <=
				moMath <
				Real >::FAbs (fRoot2 - fT22) ? fRoot1 : fRoot2);
			Real fZ = rkA[0][1];
...
// c * M
template < class Real > inline moMatrix3 < Real > operator* (Real fScalar,
const
moMatrix3 <
Real > &rkM)
{
...
// v^T * M
template < class Real > inline moVector3 < Real >
operator* (const moVector3 < Real > &rkV, const moMatrix3 < Real > &rkM)
{
	return moVector3 < Real >
