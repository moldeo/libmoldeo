...
{
	LST_VIDEOCAMERA, LST_UNKNOWN
};

/// Sistema de tratamiento de una seal en vivo de video
/**
 * Sistema de tratamiento de una seal en vivo de video
 *   Por cada cmara se crea una instancia de esta clase,
...
class LIBMOLDEO_API moLiveSystem
{
	...
	private:moLiveSystemType m_Type;
	moText m_CodeName;
	moText m_DeviceName;
...
/// Sistema de tratamiento de seales en vivo de video
/**
 * Sistema de tratamiento de seales en vivo de video
 *   Administra varias instancias de seales en vivo
 */
class LIBMOLDEO_API moLiveSystems:public moLiveSystemPtrs
{
	public:moLiveSystems ();
	...
	protected:moVideoFramework * m_pVideoFramework;
};

/// Clase de manipulacin para el cuadro de un video en memoria
/**
 * Clase de manipulacin para el cuadro de un video en memoria
 *
 */
...
{
	public:moVideoBuffer ();
	virtual ~ moVideoBuffer ();
	virtual MOboolean Init ();
	virtual MOboolean Init (moText bufferpath, moText bufferformat,
		moResourceManager * p_pResourceManager,
		MOuint width, MOuint height, MOuint xsource,
		MOuint ysource, MOuint sourcewidth, MOuint sourceheight,
...
/// Buffer Circular de imgenes para video
/**
	Este buffer permite cargar de manera gradual una sucesin de imgenes
	manteniendo siempre una cantidad fija histrica de cuadros para ser reproducidos desde la memoria.
	El circular buffer comprime en el formato seleccionado cada cuadro.

	Un uso comn es el de aplicar un circularbuffer a una cmara para grabar en memoria una cantidad de cuadros
...
{
	public:moCircularVideoBuffer ();
	virtual ~ moCircularVideoBuffer ();
	virtual MOboolean Init ();
	virtual MOboolean Init (moText videoinput, moText bufferformat,
		moResourceManager * p_pResourceManager, MOint frames,
		MOint width, MOint height,
		MOint xsource, MOint ysource,
		MOint sourcewidth, MOint sourceheight);
	virtual MOboolean Finish ();
	virtual void GetFrame (MOuint p_i);
...
moCircularVideoBuffers)  class moDirectory;
class LIBMOLDEO_API moVideoBufferPath:public moAbstract
{
	public:moVideoBufferPath ();
	virtual ~ moVideoBufferPath ();
	virtual MOboolean Init ();
	virtual MOboolean Init (moResourceManager * pResources,
...
class LIBMOLDEO_API moVideoManager:public moResource
{
	...
	protected:moLiveSystems * m_pLiveSystems;
