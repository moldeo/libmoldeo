...
// moVector3 class ------------------------------------------------------------
template < class Real > class LIBMOLDEO_API moVector3:public moAbstract
{
	public:
		// construction
		moVector3 ()
		...
		{
			moVector3 < Real > ret (m_afTuple[0] + rkV.m_afTuple[0],
				m_afTuple[1] + rkV.m_afTuple[1],
				m_afTuple[2] + rkV.m_afTuple[2]);
			return ret;
		}
		moVector3 operator- (const moVector3 & rkV) const
		{
			return moVector3 < Real > (m_afTuple[0] - rkV.m_afTuple[0],
				m_afTuple[1] - rkV.m_afTuple[1],
				m_afTuple[2] - rkV.m_afTuple[2]);
		}
		moVector3 operator* (Real fScalar) const
		{
			return moVector3 < Real > (fScalar * m_afTuple[0],
				fScalar * m_afTuple[1],
				fScalar * m_afTuple[2]);
		}
		moVector3 operator/ (Real fScalar) const
		...
		Real Length ()const
		{
			return moMath < Real >::Sqrt (m_afTuple[0] * m_afTuple[0] +
		...
		Real Dot (const moVector3 & rkV) const
		{
			return m_afTuple[0] * rkV.m_afTuple[0] +
				m_afTuple[1] * rkV.m_afTuple[1] + m_afTuple[2] * rkV.m_afTuple[2];
		}
		Real Normalize ()
		{
			Real fLength = Length ();
		...
		{
			moVector3 < Real >
				kCross (m_afTuple[1] * rkV.m_afTuple[2] -
				m_afTuple[2] * rkV.m_afTuple[1],
				m_afTuple[2] * rkV.m_afTuple[0] -
				m_afTuple[0] * rkV.m_afTuple[2],
				m_afTuple[0] * rkV.m_afTuple[1] -
				m_afTuple[1] * rkV.m_afTuple[0]);
			return kCross;
		}
			...
			const moVector3 < Real > &rkV3, Real afBary[4]) const
		{
			...
			{
				rkV0 - rkV3, rkV1 - rkV3, rkV2 - rkV3, *this - rkV3
			};
		...
		{
			Real fInvLength;
			if (moMath < Real >::FAbs (rkW.m_afTuple[0]) >= moMath <
				Real >::FAbs (rkW.m_afTuple[1]))

			{
				...
				fInvLength =
					moMath <
					Real >::InvSqrt (rkW.m_afTuple[0] * rkW.m_afTuple[0] +
				...
				rkV.m_afTuple[1] =
					rkW.m_afTuple[2] * rkU.m_afTuple[0] -
...
// arithmetic operations
template < class Real > inline moVector3 < Real > operator* (Real fScalar,
const
moVector3 <
Real > &rkV)
{
	return moVector3 < Real > (fScalar * rkV[0], fScalar * rkV[1],
		fScalar * rkV[2]);
}
