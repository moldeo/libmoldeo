...
{								 /// TEXTURA BASE
	MO_TYPE_TEXTURE,
	MO_TYPE_TEXTURE_MULTIPLE,	 /// textura mltiple
	MO_TYPE_MOVIE,				 /// pelcula
...
/**
 * Clase que encapsula una textura openGL y provee funcionalidades bsicas tales como copiar contenidos desde o hacia
 * bfer, leer desde un archivo, manejar el FBO al cual pueda estar asociada la textura, etc.
...
class LIBMOLDEO_API moTexture:public moAbstract
{
	public:
		...
		/**
		 * Inicializa propiedades bsicas de la textura tales como el nombre, id, administradores y parmetros.
		 ...
		 * @param p_param parmetros de textura.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		virtual MOboolean Init (moText p_name, MOuint p_moid,
			moResourceManager * p_res, moTexParam p_param =
			MODefTex2DParams);
		...
		/**
		 * Construye una textura vaca con el ancho y alto especificados.
		 ...
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean BuildFromBuffer (MOuint p_width, MOuint p_height,
			GLvoid * p_buffer, GLenum p_format =
			GL_RGBA, GLenum p_type = GL_UNSIGNED_BYTE);

		/**
		 * Construye una textura a partir de la imgen contenida en p_filename.
		 * @param p_filename nombre del archivo que contiene la imgen a cargar en la textura.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean BuildFromFile (moText p_filename);

		/**
		 * Revisa que el archivo especificado con p_filename sea un archivo de imgen soportado. En estos momentos, los
		...
		/**
		 * Construye una textura a partir de la imgen especificada en los parmetros p_param.
		 * @param p_param puntero a un objeto de parmetros que contiene el nombre del archivo del cual la textura ser leida.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		...
		/**
		 * Copia a la textura el bfer pasado como parametro, asumiendo que el mismo tiene el alto y ancho correcto.
		 * @param p_buffer puntero al bfer en memoria que ser copiado a la textura.
		 * @param p_format formato de los datos contenidos en el bfer (GL_RGBA, GL_RGB, etc.).
		 * @param p_type tipo de los datos contenidos en el bfer (GL_UNSIGNED_BYTE, GL_FLOAT, etc.).
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean SetBuffer (GLvoid * p_buffer, GLenum p_format =
			GL_RGBA, GLenum p_type = GL_UNSIGNED_BYTE);

		/**
		 * Copia a la textura el bfer pasado como parametro, y usando los parametros de alto y ancho como las nuevas dimensiones de la textura.
		 * @param p_width ancho de los datos contenidos en el bfer.
		 * @param p_height alto de los datos contenidos en el bfer.
		 * @param p_buffer puntero al bfer en memoria que ser copiado a la textura.
		 * @param p_format formato de los datos contenidos en el bfer (GL_RGBA, GL_RGB, etc.).
		 * @param p_type tipo de los datos contenidos en el bfer (GL_UNSIGNED_BYTE, GL_FLOAT, etc.).
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean SetBuffer (MOuint p_width, MOuint p_height, GLvoid * p_buffer,
			GLenum p_format = GL_RGBA, GLenum p_type =
			GL_UNSIGNED_BYTE);

		/**
		 * Copia el contenido actual de la textura al bfer pasado como parametro.
		 * @param p_buffer puntero al bfer en memoria donde ser copiada la textura.
		 * @param p_format especifica el formato de pxel de los datos copiados al bfer (GL_RGBA, GL_RGB, etc.).
		 * @param p_type especifica el tipo de pxel de los datos copiados al bfer (GL_UNSIGNED_BYTE, GL_FLOAT, etc.).
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean GetBuffer (GLvoid * p_buffer, GLenum p_format =
			GL_RGBA, GLenum p_type = GL_UNSIGNED_BYTE);
		...
		/**
		 * Utiliza p_fbo como nuevo objeto de Framebuffer para la textura, adems de generar un nuevo
		...
		{
			m_fbo_attach_point = p_fbo_attach_point;
		}
		/**
		 ...
		 * @return ndice del punto de attachement (0 para GL_COLOR_ATTACHMENT0_EXT, 1 para GL_COLOR_ATTACHMENT1_EXT, etc.).
		 */
		MOuint GetFBOAttachPoint ()
		{
			return m_fbo_attach_point;
		}

		/**
		 * Devuelve el tipo de pxel de la textura.
		 * @return tipo de pxel (GL_UNSIGNED_BYTE, GL_FLOAT, etc.).
		 */
		MOuint GetType ()
		{
			return m_type;
		 ...
		 * devuelve el ancho origial escalado por el rango mximo de la coordenada S de texturas.
		 ...
		 * devuelve la altura origial escalada por el rango mximo de la coordenada T de texturas.
		...
		/**
		 * Devuelve el nmero de componentes por pxel.
		...
		/**
		 * Devuelve el nmero total de txels (alto x ancho x nmero de componentes).
		 * @return nmero de txels.
		 */
		MOlong GetTexelCount ()
		{
			return m_width * m_height * m_components;
		}
		 ...
		 */
		MOlong GetDataTexelCount ()
		{
			return GetDataWidth () * GetDataHeight () * m_components;
		}

		/**
		 * Devuelve el valor mximo de la coordenada S de textura.
		 * @return valor mximo de S.
		 */
		MOfloat GetMaxCoordS ()
		{
			return m_max_coord_s;
		}

		/**
		 * Devuelve el valor mximo de la coordenada T de textura.
		 * @return valor mximo de T.
		 */
		MOfloat GetMaxCoordT ()
		{
			return m_max_coord_t;
		}

		/**
		 * Devuelve los parmetros de la textura.
		...
		/**
		 * Devuelve el trget OpenGL de la textura (GL_TEXTURE_2D, GL_TEXTURE_RECT, etc.).
		...
		/**
		 * Devuelve el formato interno de la textura, es decir, el nmero de componentes de color de la misma
		 * (GL_RGBA, GL_RGB, etc.).
		 * @return formato interno de la textura.
		 */
		GLenum GetTexInternalFormat ()
		{
			return m_param.internal_format;
		}
		...
		GLint GetMinFilter ()
		{
			return m_param.min_filter;
		}

		/**
		 * Devuelve el mtodo utilizado en el filtro de magnificacin (GL_NEAREST o GL_LINEAR).
		 * @return mtodo de filtrado de magnificacin.
		 */
		GLint GetMagFilter ()
		{
			return m_param.mag_filter;
		}

		/**
		 * Devuelve el mtodo de envolvimiento utilizado en la coordenada S de textura (GL_CLAMP, GL_REPEAT).
		 * @return mtodo de envolvimiento en coordenada S.
		 */
		GLint GetWrapS ()
		{
			return m_param.wrap_s;
		}

		/**
		 * Devuelve el mtodo de envolvimiento utilizado en la coordenada T de textura (GL_CLAMP, GL_REPEAT).
		 * @return mtodo de envolvimiento en coordenada T.
		 */
		GLint GetWrapT ()
		{
			return m_param.wrap_t;
		}

		/**
		 * Invierte verticalmente el bfer de imgen apuntado por pBuffer.
		 * @param pBuffer puntero al bffer que contiene la imgen a invertir.
		 * @param p_depth nmero de componentes de color por pxel.
		 */
		 ...
		 */
		void CopyFromTex (moTexture * p_src_tex, MOboolean p_copy_glid,
			MOboolean p_copy_moid, MOboolean p_copy_type,
		...
		/**
		 *   Salva al disco la textura especificando el nuevo tamao y el formato
		 ...
		 *   @return el nombre del archivo si fue salvado exitosamente, sino una cadena vaca moText("")
		 */
		moText CreateThumbnail (moText p_bufferformat, int w, int h,
			moText newfilename = moText (""));
		bool CalculateLuminanceAndConstrast (int x0 = 0, int y0 = 0, int x1 =
			0, int y1 = 0);
		int GetLuminance ()
		{
			return Luminance;
	...
	protected:MOboolean m_bBuildedFromFile;
	moFile * m_pFile;
...
class LIBMOLDEO_API moTextureMemory:public moTexture
{
	public:
		...
		virtual MOboolean Init (moText p_name, MOuint p_moid,
			...
			MODefTex2DParams);
		virtual MOboolean Init (moText p_name, MOuint p_moid,
			moResourceManager * p_res, moText bufferformat,
			moBitmap * pImageResult, moTexParam p_param =
			MODefTex2DParams);
		...
		virtual MOboolean BuildFromBitmap (moBitmap * p_bitmap,
...
/// una textura asociada a una animacin de cuadros
/**
...
class LIBMOLDEO_API moTextureAnimated:public moTexture
{
	public:
		...
		/**
		 * Devuelve el OpenGL ID de textura que corresponde al tempo pasado como parmetro.
		...
		/**
		 * Devuelve el OpenGL ID de la textura, pasando como parmetro una fraccin de ciclo 0 = cuadro 0, 1.0 = ltimo cuadro.
		...
		/**
		 * Devuelve el nmero de cuadros de la animacin.
		 * @return nmero de cuadros.
		 */
		virtual MOuint GetNumberFrames ()
		{
			return m_nFrames;
		}

		/**
		 * Fija el nmero de cuadros de la animacin
		 * @param p_nframes Nuevo nmero de cuadros.
		 */
		virtual void SetNumberFrames (MOuint p_nframes)
		{
			m_nFrames = p_nframes;
		}						 //

		/**
		 * Devuelve los cuadros por segundo de la animacin.
		...
		/**
		 * Fija el nmero de cuadros por segundo de la animacin.
		 * @param p_fps Nuevo nmero de cuadros por segundo de la animacin.
		 */
		virtual void SetFramesPerSecond (MOfloat p_fps)
		{
			m_fFramesPerSecond = p_fps;
		}
		/**
		 * Calcula el cuadro i-simo de la animacin.
		 * @param p_i ndice del cuadro a cualcular.
		 */
		virtual void GetFrame (MOuint p_i);

		/**
		 * Configura el modo de interpolacin de la textura. Este modo genera texturas intermedias que
		 * entre dos cuadros de la animacin.
		 * @param p_FrameJump cuadro que define el "salto" de la interpolacin: las texturas interpoladas son generadas
		 * entre el cuadro p_FrameJump y p_FrameJump + 1.
		 * @param p_InterpolationTime el tiempo que dura la interpolacin.
		 */
		virtual void SetInterpolation (MOuint p_FrameJump,
			MOuint p_InterpolationTime);

		/**
		 * Devuelve si el cuadro de la animacin est en el rango de interpolacin.
		 * @return true o falsed dependiendo del cuadro actual.
		 */
		virtual MOboolean IsInterpolating ();

		/**
		 * Devuelve si el modo de interpolacin est activo.
		 * @return true o false dependiendo del estado del modo de interpolacin.
		 */
		virtual MOboolean IsInterpolationActive ();

		/**
		 * Fija el estado del modo de interpolacin.
		 * @param activate el nuevo estado.
		 * @return el estado del modo de interpolacin. Aunque se intente activar la interplacin, puede ocurrir
		 * que los shaders requeridos para la operacin de interpolacin no estn disponibles, en cuyo caso se
		 * devuelve false.
		 */
		virtual MOboolean ActivateInterpolation (MOboolean activate = true);
	protected:virtual MOboolean NeedsInterpolation ();
	...
	MOuint m_FrameJump;			 /* salto entre el cuadro inicial y final mnimo, para resolver la interpolacin */
	MOuint m_InterpolationTime;	 /*intervalo de resolucin de la interpolacin en milisegundos */

	//interpolation variables
	MOuint m_StartTime;
	MOuint m_FrameStart;		 /*cuadro inicial de la interpolacin */
	MOuint m_FrameEnd;			 /*cuadro final de la interpolacin */
								 /*valor entre 0 y 1 indicando el nivel de interpolacin, 0 corresponde al cuadro inicial, 1 al cuadro final */
...
class LIBMOLDEO_API moTextureMultiple:public moTextureAnimated
{
	public:
		...
		/**
		 * Revisa que el archivo especificado con p_filename sea un archivo de textura mltiple soportado.
		 * @return true si el archivo est soportado, false en caso contrario.
		 */
		MOboolean SupportedFile (moText p_filename);

		/**
		 * Construye una textura mltiple a partir de la imgenes especificadas en los parmetros p_param.
		 * @param p_param puntero a un objeto de parmetros que contiene el nombre del archivo del cual la textura ser leida.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean Load (moParam * p_param);
		...
		/**
		 * Construye una textura mltiple a partir de la imgenes especificadas en los subvalores del valor p_value
		 * @param p_value puntero a un objeto de subvalores que contiene el nombre de los archivos de los cuales las texturas sern leidas.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean Load (moValue * p_value);

		/**
		 * Construye una textura mltiple a partir de las texturas entre los ndices p_id0 y p_id1 de la lista
		 * de texturas p_textures.
		 * @param p_name Nombre que es asignado a la textura.
		 * @param p_textures Referencia a la lista de texturas de donde son extradas las texturas a copiar.
		 * @param p_id0 ndice inicial.
		 * @param p_id1 ndice final.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean Load (moText p_name, moTextureArray & p_textures, MOuint p_id0,
			MOuint p_id1);

		/**
		 * Inicializa la textura mltiple con p_tex_count en blano.
		 * @param p_tex_count nmero de texturas.
		 */
		void SetTextureCount (MOuint p_tex_count);

		/**
		 * Devuelve el nmero de texturas en la secuencia que define esta textura mltiple.
		...
		/**
		 * Fija como textura actual aquella que tiene la posicin i-sima en la secuencia.
		 * @param p_i ndice de la textura a fijar como actual.
		 */
		void GetFrame (MOuint p_i);

		/**
		 * Copia la textura apuntada por p_texture en la posicin i-sima de la secuencia.
		 * @param p_i ndice de la textura donde se copiara la que es pasada como parmetro.
		 * @param p_texture puntero al objeto de textura a copiar en la posicin i-sima.
		 */
		...
		/**
		 * Devuelve un puntero al objeto de textura en la posicin i-sima en la secuencia.
		 * @param p_i ndice de la textura requerida.
		 * @return puntero a la textura devuelta.
		 */
		moTexture *GetTexture (MOuint p_i)
		{
			return m_textures_array[p_i];
		}

		/**
		 * Valida el ndice de textura pasado como parmetro, es decir, revisa que est entre los rngos
		 * permitidos (0 y nmero total de texturas en esta textura mltiple - 1).
	...
	protected:moTextureArray m_textures_array;
};

///  una textura animada basada en una pelcula
/**
 * Clase para manejar una textura animada basada en una pelcula.
 *
 */
class LIBMOLDEO_API moMovie:public moTextureAnimated
{
	public:
		...
		/**
		 * Revisa que el archivo especificado con p_filename sea un archivo de pelcula soportado.
		 * Tiene que ser de extensin avi, mpg o mov.
		 * @return true si el archivo est soportado, false en caso contrario.
		 */
		MOboolean SupportedFile (moText p_filename);

		/**
		 * Carga en la textura la pelcula p_filename.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean LoadMovieFile (moText p_filename);

		/**
		 * Construye una pelcula a partir de los parmetros p_param.
		 * @param p_param puntero a un objeto de parmetros que contiene el nombre del archivo del cual la pelcula ser leida.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		MOboolean Load (moParam * p_param);
		MOboolean Load (moValue * p_value);
	...
	protected:void GetFrame (MOuint p_i);
	MOint frameprevious;
	MOint lastframe;
