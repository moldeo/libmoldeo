...
{
	public:moHueSatIntMatrix ();
	~moHueSatIntMatrix ();
	void Init (MOboolean p_PreserveLuminance = true, MOfloat p_min_int =
		0.0, MOfloat p_max_int = 2.0, MOfloat p_min_sat =
		0.0, MOfloat p_max_sat = 2.0, MOfloat p_min_hue =
		0.0, MOfloat p_max_hue = 2 * moMathf::PI);
	void Finish ();
	...
	protected:MOboolean m_PreserveLuminance;
...
{
	public:moBrightContMatrix ();
	~moBrightContMatrix ();
	void Init (MOfloat p_min_bright = 0.0, MOfloat p_max_bright =
		2.0, MOfloat p_min_cont = 0.0, MOfloat p_max_cont = 2.0);
	void Finish ();
	void Update (MOfloat p_bright, MOfloat p_cont);
	protected:MOfloat m_bright, m_cont;
...
/**
 * Esta clase define un filtro 2D para aplicar en texturas. Un filtro es bsicamente
 * un programa de shader (escrito en CG o GLSL) con un conjunto de predefinido de parmetros
 * uniformes y una grilla 2D donde las texturas fuente son mapeadas. Los puntos de la grilla 2D
 * pueden ser modificados en el estadio de vrtices del filtro, permitiendo distorsiones arbitrarias
...
class LIBMOLDEO_API moTextureFilter:public moAbstract
{
	public:
		 ...
		 * @param p_src_tex referencia a la lista de texturas de orgen.
		 ...
		 * @param p_params puntero al objeto de parmetros a utilizar en este filtro.
		 * @return true si la operacin fue exitosa, false en caso contrario.
		 */
		virtual MOboolean Init (moGLManager * p_glman,
			moRenderManager * p_renderman,
			moTextureArray & p_src_tex,
			moTextureArray & p_dest_tex, moShader * p_shader,
			moTextFilterParam * p_params = NULL);
		...
		/**
		 * Aplica el filtro sobre las texturas de orgen y escribiendo el resutlado en las texturas de destino.
		 * @param p_i cuadro a utilizar en las texturas de orgen (en el caso de que sean animadas).
		 * @param p_fade constante de "fading" utilizada para mezclar el resultado del filtro.
		 * @param p_params parmetros del filtro.
		 */
		void Apply (MOuint p_i, MOfloat p_fade = 1.0, moTextFilterParam * p_params =
			NULL);

		/**
		 * Aplica el filtro sobre las texturas de orgen y escribiendo el resutlado en las texturas de destino.
		 * @param p_cycle fraccin del ciclo entre 0.0 y 1.0 a utilizar en las texturas de orgen (en el caso de que sean animadas).
		 * @param p_fade constante de "fading" utilizada para mezclar el resultado del filtro.
		 * @param p_params parmetros del filtro.
		 */
		void Apply (MOfloat p_cycle, MOfloat p_fade =
			1.0, moTextFilterParam * p_params = NULL);

		/**
		 * Aplica el filtro sobre las texturas de orgen y escribiendo el resutlado en las texturas de destino.
		 * @param p_tempo tempo a utilizar en las texturas de orgen (en el caso de que sean animadas).
		 * @param p_fade constante de "fading" utilizada para mezclar el resultado del filtro.
		 * @param p_params parmetros del filtro.
		 */
		void Apply (moTempo * p_tempo, MOfloat p_fade =
			1.0, moTextFilterParam * p_params = NULL);

		/**
		 * Devuelve el puntero a la lista de texturas de orgen.
		 * @return puntero a la lista de texturas de orgen.
		 */
		...
		/**
		 * Verifica que todas las texturas de destino estn asociadas a lo bfers de color del mismo FBO. En caso
		 * contrario, las mismas sern re-asociadas (utilizando el FBO de la primera textura de destino) en el
	...
	void SetupShader (MOint w, MOint h, moTempo * p_tempo, MOfloat p_fade,
