...
typedef enum
{
	MO_MODEL_X3D = 0, MO_MODEL_3DS = 1, MO_MODEL_OBJ = 2,
       	MO_MODEL_MD2 = 3, MO_MODEL_MD3 = 4
} mo3dModelType;
...
struct mo3dModel
{
	...
	vector < moMaterialInfo > pMaterials;
								 // The object list for our model
	vector < mo3dObject > pObject;
};

/// Clase para un nodo de una escena
/**
 * Una nodo de escena es el objeto principal que contiene una jerarqua de objetos 3d
...
class LIBMOLDEO_API moSceneNode:public moAbstract
{
	...
	protected:void *SceneNodeImplementation;
};

/// Clase base para los modelos tridimensionales
/**
 * De esta clase deberan derivar los objetos tridimensionales importados por los recursos
 *
 */
class LIBMOLDEO_API mo3DModelSceneNode:public moSceneNode
{
	public:mo3DModelSceneNode ();
...
class LIBMOLDEO_API moScene:public moAbstract
{
	...
	virtual void Update ();
	virtual void Interaction ();
	protected:void *SceneImplementation;
};
...
{
	public:moTextureManager * Textures;
	MOuint nModels;
	MOuint nMaxModels;
	mo3dModel ** Models;
	mo3dModelManager ();
...
class LIBMOLDEO_API mo3dModelManagerRef:public moAbstract
{
	...
	private:void LoadModels (moConfig *, MOuint, mo3dModelManager *);
};
