...
// moIntegrate1 class -----------------------------------------------------------
template < class Real > class LIBMOLDEO_API moIntegrate1:public moAbstract
{
	public:
			...
			void *pvUserData = 0)
		{
			...
			aafRom = new Real *[2];
			for (int i = 0; i < 2; i++)
				aafRom[i] = new Real[iOrder];

			Real fH = fB - fA;

			aafRom[0][0] =
				((Real) 0.5) * fH * (oF (fA, pvUserData) + oF (fB, pvUserData));
			for (int i0 = 2, iP0 = 1; i0 <= iOrder; i0++, iP0 *= 2, fH *= (Real) 0.5)
			{
				...
				{
					aafRom[1][i2] =
						(iP2 * aafRom[1][i2 - 1] - aafRom[0][i2 - 1]) / (iP2 - 1);
				}
...
// moMinimize1 class -----------------------------------------------------------
template < class Real > class LIBMOLDEO_API moMinimize1:public moAbstract
{
	public:typedef Real (*Function) (Real, void *);
	...
	void GetMinimum (Real fT0, Real fF0, Real fTm, Real fFm, Real fT1,
		Real fF1, int iLevel);
	void GetBracketedMinimum (Real fT0, Real fF0, Real fTm, Real fFm,
